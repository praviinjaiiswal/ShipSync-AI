...
export function NewsTicker({ items, isLoading = false, onSelectHeadline }: NewsTickerProps) {
  ...
  if (!items || items.length === 0) {
    return (
      <div className="w-full bg-slate-950 border-b border-slate-800/80 text-white h-10 flex items-center px-4 overflow-hidden select-none z-40 relative">
        <div className="flex items-center gap-2 bg-slate-800 text-slate-300 text-[11px] font-bold uppercase tracking-wider px-3 h-full shrink-0 border-r border-slate-700/60">
        ...
        <div className="ml-3.5 flex items-center gap-2 text-xs text-slate-400">
          <span className="inline-block w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
          <span>Live sync in progress — first regulatory updates will appear here shortly</span>
        </div>
      </div>
    );
  }
