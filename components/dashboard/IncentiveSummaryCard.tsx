    ...
    <div className="border border-emerald-500/30 bg-emerald-500/5 rounded-lg p-6">
      ...
      <div className="flex items-center gap-1">
        ...
        <span className="text-3xl font-bold text-emerald-600 dark:text-emerald-400">
          {data.totalEstimated.toLocaleString(undefined, { maximumFractionDigits: 0 })}
        </span>
      </div>
      {data.unmatchedCount > 0 && (
        <p className="text-xs text-muted-foreground mt-2 flex items-center gap-1">
          <AlertCircle className="w-3.5 h-3.5" />
          {data.unmatchedCount} shipments ke HS codes ke liye rate missing hai — Settings me DGFT CSV import karo.
        </p>
      )}
